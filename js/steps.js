	...
	_steps.prototype = {
		...
		render: function() { // 渲染样式
			...
			options.data.forEach(function(currentValue, index, array) {
				(currentValue[options.props.status] || currentValue[options.props.status] == 0) && (options.dataSetStatus =
					true);
				...
				var stepClass = currentValue[options.props.status] ?
					...
					((!options.dataSetStatus && index <= options.active) ?
						options.finishClass :
						"");
				stepContainer = stepContainer + '<div class="step ' + options.defaultClass + ' ' + stepClass + '" style="' +
					_this.getStepStyle(index).join("") + '">{{stepHtml}}</div>';
				...
				var stepLineBox = '<div class="step-line-box" style="' + lineStyle + 'order:' + lineIndex +
					'"><div class="step-line {{finishLineClass}}"></div><div class="step-icon ' + stepIconClass +
					'"><div class="' + stepIconInnerClass + '">' + stepIconInnerText + '</div></div></div>';
				var stepTitle = '<div class="step-title" style="' + titleStyle + 'order:' + titleIndex + '">' + currentValue[
					options.props.title] + '</div>';
				var stepDesc = '<div class="step-description" style="' + descStyle + 'order:' + descIndex + '">' +
					currentValue[options.props.description] + '</div>';
					
				// var cusHtml = '<div class="step-description" style="' + descStyle + 'order:' + descIndex + '">' + currentValue[
				// 	options.props.customHtml] + '</div>';
